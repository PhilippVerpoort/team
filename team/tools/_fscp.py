...
def calc_FSCP(
    ...
    if options is None:
        options_cost = [
            m.group(1)
            for var in data
            if (m := match(rf"{var_cost}\|(.*)", var))
        ]
        options_ghgi = [
            m.group(1)
            for var in data
            if (m := match(rf"{var_ghgi}\|(.*)", var))
        ]

        options_cost_notin_ghgi = [
            o for o in options_cost if o not in options_ghgi
        ]
        options_ghgi_notin_cost = [
            o for o in options_ghgi if o not in options_cost
        ]
