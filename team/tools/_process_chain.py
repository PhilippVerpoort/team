...
class ProcessChain:
    ...
    def __init__(
        ...
        if process_diagram is None and process_tree is None:
            raise Exception(
                "Either the process_diagram or the process_tree "
                "argument must be provided."
            )
        if process_diagram is not None and process_tree is not None:
            raise Exception(
                "The process_diagram and process_tree arguments "
                "cannot both be provided."
            )

        self._proc_graph = (
            self._read_diagram(process_diagram)
            if process_diagram is not None
            else process_tree
        )
        self._flows = list(
            {
    ...
    def igraph(self) -> tuple[Graph, Layout]:
        if not HAS_IGRAPH:
            raise ImportError(
                "Need to install the `igraph` package first. "
    ...
    def _reduce_subdiagram(subdiagram: str) -> tuple[str, str, str]:
        ...
        for i in range(len(processes)):
            ...
            if flow is None and i + 1 < len(processes):
                raise Exception(
                    f"Flow must be provided for processes feeding "
    ...
    def calc_scaling(
        self, data: dict[str, float | Q | pd.Series] | pd.DataFrame, **kwargs
    ):
            ...
            df = (
                pd.DataFrame(
                    ...
                    index=(
                        [0]
                        if all(
                            not isinstance(c, pd.Series) for c in data.values()
                        )
                        else None
                    ),
                )
    ...
    def calc_scaling_single(
        ...
        tsm = np.array(
            [
                [
                    ...
                    if proc1 == proc2
                    else -row[pat_input(proc2, flow)]
                    .to(row[pat_output(proc1, flow)].u)
                    .m
                    if proc2 in proc1_flow_targets
                    else 0.0
        ...
        fu = np.array(
            [
                func_unit[proc1][flow].to(row[pat_output(proc1, flow)].u).m
                if proc1 in func_unit and flow in func_unit[proc1]
                else 0.0
                for proc1 in self._proc_graph
                for flow, proc1_flow_targets in self._proc_graph[proc1].items()
            ]
