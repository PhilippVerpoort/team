...
def calc_emissions(
    ...
    """Calculate emissions from inputs, outputs, and emission factors.
    ...
    # Loop over existing variables to find inputs/outputs.
    unused = []
    for io in data:
        # Check if variable is input/output exists.
        m = match(r"^(Input|Output)\|(.*)$", io)
        if not m:
            continue
        # Split variable up into type (Input/Output) and flow (H2, CH4, etc).
        io_type, io_flow = m.group(1), m.group(2)
        # Loop over existing variables to find
        is_unused = True
        for var in data:
            m = match(rf"^Emission Factor\|{io_flow}(\|.*)?$", var)
            # Check if variable is emission factor
            if not m:
                continue
            is_unused = False
            # Get value from data.
            emi_fac = data[m.group(0)]
            ...
            emi_name = f"GHG Emissions|{io_flow}{m.group(1)}"
            # Compute and assign.
...
def calc_GHGI(
    ...
    """Calculate emissions intensity of a product or service.
    ...
    data : dict[str, float | Q | pd.Series] | pd.DataFrame
        A dict or dataframe on which to perform the calculation.
    name : str, optional
        Name of the product or service for which the greenhouse-gas intensity
        is calculated. By default, the name is left empty.
    reference : str, optional
        The reference activity variable by which the final emissions intensity
...
def calc_GHGI_pc(
    ...
    **kwargs,
    ...
    """Calculate emissions intensity of a product/service for a process chain.
    ...
    **kwargs
        Keyword arguments for `calc_GHGI`.
