...
def calc_LCOX(
    ...
    if capex is not None or fopex is not None:
        ...
        if cap_ref is not None and cap_ref_io is not None:
            if capex is not None:
                if interest_rate and book_lifetime:
    ...
    return {key: value / ref for key, value in ret.items()}
    ...
    **kwargs,
    ...
    for proc in processes:
        ...
        if fu is not None:
            factor /= fu
        if name is not None:
            prefix_new = f"{name}|{proc}|"
        else:
            prefix_new = f"{proc}|"
        ret |= {
            (prefix_new + var_name): var_value * factor
