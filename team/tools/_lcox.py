...
import pandas as pd
from cet_units import Q, ureg


@ureg.wraps(ureg.dimensionless / ureg.year, (ureg.dimensionless, ureg.year))
def annuity_factor(ir: Q, bl: Q) -> Q:
    """Calculate annuity factor from interest rate and book lifetime.

    Also sometimes referred to as the capital recovery factor (CRF).

    Parameters
    ----------
    ir : Q
        Interest rate. Should be a float or dimensionless pint quantity object.
    bl : Q
        Book lifetime. Should be a pint quantity object of dimension `[time]`.

    Returns
    -------
    Q
        The annuity factor per year computed from interest rate and book
        lifetime. The return is a pint quantity object of dimension `1/[time]`.
        When multiplied with the CAPEX, it gives the annualised capital cost.
...
def calc_LCOX(
...
) -> dict[str, float | Q | pd.Series]:
    """Calculate levelised costs of a product or service.

    Parameters
    ----------
    data : dict[str, float | Q | pd.Series] | pd.DataFrame
        A dict or dataframe on which to perform the calculation.
    name : str, optional
        Name of the product or service for which the levelised cost is
        calculated. By default, the name is left empty.
    reference : str, optional
        The reference activity variable by which the final levelised cost
        should be divided. By default, no division is performed.
    interest_rate : float | Q, optional
        The interest rate to assume in the cost calculation. If none is
        provided, `data` is searched for `Interest Rate`. If this cannot be
        found either, the capital costs cannot be computed.
    book_lifetime : float | Q, optional
        The book lifetime to assume in the cost calculation. If none is
        provided, `data` is searched for `Book Lifetime` or `Lifetime`. If
        this cannot be found either, the capital costs cannot be computed.

    Returns
    -------
    dict[str, float | Q | pd.Series]
        A dict containing the calculations.
    ...
    book_lifetime = (
        book_lifetime or data.get("Book Lifetime") or data.get("Lifetime")
    )
    ...
    for io in data:
        ...
        if not match(r"^(Input|Output)\|.*", io):
            continue
        # Split variable into type (Input/Output) and flow (Elec, H2, CH4 etc).
        io_type, io_flow = io.split("|", 2)
        # Get corresponding price for flow. If not found, report as unused.
...
def calc_LCOX_pc(
    ...
    **kwargs,
    ...
    """Calculate levelised costs of a product/service for a process chain.
    ...
    reference : str, optional
        The reference activity variable by which the final levelised cost
        should be divided. By default, no division is performed.
    pat : Callable, optional
        Pattern for finding the respective process parameters in the data.
        By default, the pattern `Tech|{process}` will be assumed.
    **kwargs
        Keyword arguments for `calc_LCOX`.
